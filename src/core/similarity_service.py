...
def get_similar_properties(external_id: str, k: int = 5, published_only: bool = True) -> list[dict] | None:
    ...
    Returns a list of payload dicts (plus a "score" key), best match
    first, excluding the source property itself. Returns None if the
    source property itself isn't found in Qdrant.
    """
    ...
    similar = [{**r.payload, "score": r.score} for r in results if r.payload.get("external_id") != external_id]
